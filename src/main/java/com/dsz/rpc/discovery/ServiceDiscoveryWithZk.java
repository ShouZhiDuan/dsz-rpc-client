...
public class ServiceDiscoveryWithZk implements IServiceDiscovery{
    ...
    public String discovery(String serviceName) {
        //完成了服务地址的查找(服务地址被删除)
        String path = "/"+serviceName; //registry/com.dsz.rpc.HelloService-version
        if(serviceRepos.isEmpty()) {
            try {
                serviceRepos = curatorFramework.getChildren().forPath(path);
                registryWatch(path);//设置服务感知
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private void registryWatch(final String path) throws Exception {
        ...
        PathChildrenCacheListener nodeCacheListener= (curatorFramework1, pathChildrenCacheEvent) -> {
            System.out.println("【客户端】收到事件：" + pathChildrenCacheEvent.getType());
            System.out.println("【客户端】发现服务注册地址有变换");
